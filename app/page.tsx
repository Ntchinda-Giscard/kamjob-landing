...
import {
  ArrowRight, Zap, Shield, Sparkles, Target, Bell, ImageIcon,
  Check, X, BadgeCheck, Briefcase, MousePointerClick, ChevronDown, Quote, User,
} from "lucide-react";

// The landing site is standalone: every CTA routes to the actual candidate app.
const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? "https://kamjob.vercel.app";
...
const FEATURE_ICONS = [MousePointerClick, ImageIcon, Zap, Sparkles, Target, Bell];
...
function Chip({ label, bg, color }: { label: string; bg: string; color: string }) {
  return (
    <span className="px-2 py-0.5 text-[10px] font-medium rounded-full" style={{ backgroundColor: bg, color }}>
      {label}
    </span>
  );
}
...
function LangSwitcher() {
  ...
  const btn = (l: Lang, label: string) => (
    <button
      ...
      style={lang === l ? { backgroundColor: GREEN, color: "white" } : { color: "var(--muted-foreground)" }}
      aria-pressed={lang === l}
    >
      {label}
    </button>
    ...
    <div className="relative mx-auto w-[280px] rounded-[2.5rem] border-8 border-foreground/90 bg-background shadow-2xl overflow-hidden">
      ...
      <div className="pt-10 pb-6 px-4">
        <div className="relative bg-card rounded-2xl shadow-xl overflow-hidden border border-border">
          <div className="h-1" style={{ background: `linear-gradient(135deg, ${GREEN} 0%, ${YELLOW} 50%, ${RED} 100%)` }} />

          <div className="absolute top-6 right-3 px-2.5 py-1 rounded-lg rotate-12 z-10" style={{ backgroundColor: GREEN }}>
            <span className="text-white font-bold text-xs tracking-wide">{t.mockup.stamp}</span>
          </div>

          <div className="p-3">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-[10px]" style={{ backgroundColor: GREEN_PALE, color: GREEN }}>
                ST
              </div>
              <span className="font-semibold text-foreground text-sm">ST Digital</span>
              <BadgeCheck className="w-4 h-4" style={{ color: GREEN, fill: GREEN_PALE }} />
            </div>
            <p className="text-sm font-bold text-foreground mb-2">Head of Business Desk</p>
            ...
            <div
              ...
              style={{ background: "linear-gradient(160deg, #1a3e72 0%, #10294e 100%)" }}
            >
              <p className="text-[10px] tracking-[0.2em] opacity-80">{t.mockup.hiringTop}</p>
              <p className="font-extrabold text-lg leading-tight">{t.mockup.hiringMain}</p>
        ...
        <div className="flex items-center justify-center gap-6 mt-4">
          <span className="w-11 h-11 rounded-full bg-card border border-border shadow-md flex items-center justify-center" style={{ color: RED }}>
            <X className="w-5 h-5" strokeWidth={3} />
          </span>
          <span className="w-11 h-11 rounded-full shadow-md flex items-center justify-center text-white" style={{ backgroundColor: GREEN }}>
    ...
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      ...
      >
        ...
        <ChevronDown
          ...
          style={{ transform: open ? "rotate(180deg)" : undefined, color: open ? GREEN : undefined }}
        />
      </button>
      {open && (
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-clip">
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-background/85 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-5 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <LangSwitcher />
            <a href={LOGIN_URL} className="hidden sm:block px-4 py-2 text-sm font-medium text-foreground rounded-full hover:bg-secondary transition-colors">
              {t.nav.login}
            </a>
            <a href={SIGNUP_URL} className="px-4 py-2 text-sm font-semibold text-white rounded-full" style={{ backgroundColor: GREEN }}>
              {t.nav.signup}
            </a>
          </div>
        </div>
      ...
      <section className="relative max-w-6xl mx-auto px-5 pt-12 pb-10 grid lg:grid-cols-2 gap-12 items-center">
        {/* decorative glows */}
        <div aria-hidden className="pointer-events-none absolute -top-10 -right-16 w-72 h-72 rounded-full blur-3xl" style={{ backgroundColor: "rgba(52,168,83,0.12)" }} />
        <div aria-hidden className="pointer-events-none absolute bottom-0 -left-20 w-64 h-64 rounded-full blur-3xl" style={{ backgroundColor: "rgba(251,188,5,0.10)" }} />
        <div className="text-center lg:text-left">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-foreground leading-tight text-balance mb-5">
            {t.hero.title} <span style={{ color: GREEN }}>{t.hero.titleAccent}</span>
          </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {t.hero.stats.map((s) => (
            <div key={s.label} className="bg-card border border-border rounded-2xl p-4 text-center">
              <p className="text-2xl font-extrabold" style={{ color: GREEN }}>{s.value}</p>
      ...
      <section className="border-y border-border bg-card/50 overflow-hidden">
        <div className="py-6">
          <p className="text-sm font-medium text-muted-foreground text-center mb-4 px-5">{t.ticker.label}</p>
          <div className="marquee-track">
            {[0, 1].map((copy) => (
              <div key={copy} className="flex shrink-0" aria-hidden={copy === 1}>
                {t.ticker.items.map((item) => (
                  <span key={`${copy}-${item}`} className="flex items-center gap-2 mx-6 text-sm font-medium text-foreground whitespace-nowrap">
        ...
        <div className="flex flex-col sm:flex-row gap-8">
          {t.how.steps.map((s, i) => (
            <div key={s.title} className="flex-1 text-center px-4">
              <div className="w-12 h-12 rounded-full mx-auto mb-3 flex items-center justify-center text-white font-bold text-lg" style={{ backgroundColor: GREEN }}>
                {i + 1}
              </div>
      ...
      <section className="max-w-6xl mx-auto px-5 pb-16 grid md:grid-cols-2 gap-5">
        <div className="bg-card border border-border rounded-3xl p-7 shadow-sm flex flex-col">
          <p className="text-xs font-bold tracking-wider mb-3" style={{ color: GREEN }}>{t.spaces.candidateLabel}</p>
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4" style={{ backgroundColor: GREEN_PALE, color: GREEN }}>
            <User className="w-6 h-6" />
          </div>
          <h2 className="text-2xl font-bold text-foreground mb-2">{t.spaces.candidateTitle}</h2>
          <p className="text-sm text-muted-foreground leading-relaxed mb-5">{t.spaces.candidateText}</p>
        ...
        <div className="bg-card border border-border rounded-3xl p-7 shadow-sm flex flex-col">
          <p className="text-xs font-bold tracking-wider mb-3" style={{ color: "#B8860B" }}>{t.spaces.employerLabel}</p>
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4" style={{ backgroundColor: YELLOW_PALE, color: "#B8860B" }}>
            <Briefcase className="w-6 h-6" />
          </div>
          <h2 className="text-2xl font-bold text-foreground mb-2">{t.spaces.employerTitle}</h2>
          <p className="text-sm text-muted-foreground leading-relaxed mb-5">{t.spaces.employerText}</p>
          <ul className="space-y-2.5 mb-7 flex-1">
      ...
      {/* Features */}
      <section id="features" className="border-y border-border bg-card/50 scroll-mt-16">
        <div className="max-w-6xl mx-auto px-5 py-16">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {t.features.items.map((f, i) => {
              ...
              return (
                <div key={f.title} className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-3" style={{ backgroundColor: c.bg, color: c.color }}>
      ...
      {/* Pricing */}
      <section id="pricing" className="max-w-6xl mx-auto px-5 py-16 scroll-mt-16">
        <h2 className="text-2xl sm:text-3xl font-bold text-foreground text-center mb-3">
          {t.pricing.title}
        </h2>
        <p className="text-muted-foreground text-center mb-10 max-w-2xl mx-auto">
          ...
          {t.pricing.plans.map((p, i) => {
            ...
            return (
              <div
                ...
                style={highlight ? { borderColor: YELLOW, borderWidth: 2 } : undefined}
              >
                {highlight && (
                  <span className="self-start text-[10px] font-bold px-2 py-1 rounded-full mb-2 text-white" style={{ backgroundColor: YELLOW }}>
                    {t.pricing.highlight}
                  </span>
                )}
                ...
                <p className="mt-2 mb-4">
                  ...
                  <span className="text-sm text-muted-foreground"> {t.pricing.perPeriod} · {p.period}</span>
                </p>
                ...
                <a
                  ...
                  style={highlight ? { backgroundColor: GREEN, color: "white" } : { backgroundColor: GREEN_PALE, color: GREEN }}
                >
                  {t.pricing.cta}
                </a>
              </div>
        ...
        <p className="text-center text-xs text-muted-foreground mt-6">{t.pricing.freeNote}</p>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-5 py-16">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {t.testimonials.items.map((tm) => (
              <figure key={tm.name} className="bg-card rounded-2xl p-6 border border-border shadow-sm">
                <Quote className="w-6 h-6 mb-3" style={{ color: GREEN, fill: GREEN_PALE }} />
      ...
      <section className="max-w-6xl mx-auto px-5 pb-16">
        ...
        >
          <div className="flex-1 text-center sm:text-left">
            <h2 className="text-2xl sm:text-3xl font-bold mb-3">{t.employers.title}</h2>
        ...
        <div className="max-w-6xl mx-auto px-5 py-10">
          <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-8">
            ...
            <nav className="text-center sm:text-left">
              <p className="text-sm font-semibold text-foreground mb-2">{t.footer.product}</p>
              <ul className="space-y-1.5 text-sm text-muted-foreground">
                ...
                <li><a href={SIGNUP_URL} className="hover:text-foreground transition-colors">{t.nav.signup}</a></li>
              </ul>
            </nav>
            <p className="max-w-xs text-sm text-muted-foreground flex items-start gap-1.5 text-center sm:text-left">
              <Shield className="w-4 h-4 shrink-0 mt-0.5" /> {t.footer.disclaimer}
            </p>
          </div>
